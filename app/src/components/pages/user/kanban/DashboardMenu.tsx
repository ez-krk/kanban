...
import { IDL } from '@/idl'
import { DOER_PDA, TEAM_PDA } from '@/models/kanban/structs'
import { newTeam } from '@/utils/api/kanban/newTeam'
import { newDoer } from '@/utils/api/kanban/newDoer'
...
type Props = {
  ...
  setSelectedTeam: React.Dispatch<React.SetStateAction<TEAM_PDA | null>>
        ...
        if (!isDisabled && db && publicKey && program) {
          ...
          if (doer) {
            ...
            addToast({
              ...
              description: t('kanban:programCreatedSuccessBody'),
            })
            ...
            <div className="flex flex-col gap-3 pb-20">
              {teams &&
                teams.map((team) => (
                  <div
                    onClick={() => {
                      setCurrentChatRoomId(team.pubkey.toString())
                      setSelectedTeam(team)
                    }}
                    key={`ChatMenu Desktop ${team.pubkey.toString()}`}
                    className={clsx(
                      currentChatRoomId === team.pubkey.toString() &&
              ...
              <div className="my-8 inline-block w-full max-w-xl -translate-y-10 transform overflow-hidden bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-gray-900">
                <div className="flex w-full flex-col pb-8">
                  ...
                  {doer ? (
                    ...
                    <div className="flex flex-grow flex-col gap-2">
                      <p className="text-center text-lg font-bold">
                        {t('kanban:registerUser')}
                      </p>
                      <div className="w-full sm:mx-auto sm:max-w-xl">
                        <p className="mx-auto mt-6 max-w-xl text-center text-sm tracking-tight text-gray-700 dark:text-gray-200">
                          {t('kanban:doerNotFound')}
                        </p>
                        <div className="gap-6  sm:px-10">
                          <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="flex flex-col gap-6  py-6 sm:px-10">
